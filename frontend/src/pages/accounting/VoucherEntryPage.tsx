...
function AccountTypeahead({ label, onChange, placeholder = "Search account…", autoFocus, onEnter }: { value: string; label: string; onChange: (id: string, label: string) => void; placeholder?: string; autoFocus?: boolean; onEnter?: () => void }) {
  ...
  const { data: results } = useQuery({
    ...
    queryFn: () => api.get("/accounting/accounts/search", { params: { q: query, limit: 12 } }).then((r) => r.data.data),
  ...
  return (
    <div className="relative w-full">
      ...
      {open && options.length > 0 && (
        <div ref={listRef} className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-52 overflow-y-auto">
          {options.map((opt: any, i: number) => (
            <div key={opt.id} className={`px-3 py-2 cursor-pointer text-sm ${i === highlight ? "bg-primary-50 text-primary-800" : "hover:bg-gray-50"}`} onMouseDown={() => select(opt)}>
...
export default function VoucherEntryPage() {
  ...
  const handleSubmit = () => {
    ...
    if (!balanced) {
      toast.error("Debit total must equal Credit total");
      return;
    }
    saveMutation.mutate({
      ...
      <div className="page-header mb-4">
        ...
        <div className="flex gap-2">
          {isEdit && !existing?.source_type && (
            <button className="btn-secondary flex items-center gap-1.5 text-sm" onClick={() => cloneMutation.mutate()} disabled={cloneMutation.isPending} title="Clone this voucher with today's date">
      ...
      <div className="card p-5 space-y-5">
        ...
        {isEdit && existing?.source_type && (
          <div className="flex items-center gap-2 px-4 py-2.5 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
            <span>⚠</span>
            <span>
              This voucher was auto-posted from <strong>{existing.source_type}</strong> and cannot be edited. Use Clone to create a correction entry.
            </span>
          </div>
        )}
            ...
            <table className="w-full text-sm">
                ...
                {lines.map((line, i) => (
                  <tr key={i} className="hover:bg-gray-50/80">
                    <td className="px-1 py-1.5">
                      <AccountTypeahead value={line.account_id} label={line.account_label} onChange={(id, lbl) => setLineAccount(i, id, lbl)} placeholder="Search account…" />
                    </td>
                    <td className="px-1 py-1.5">
                      <input type="number" min="0" step="0.01" className="input py-1.5 text-sm text-right w-full" value={line.debit_amount} onChange={(e) => autoFillDiff(i, "debit_amount", e.target.value)} placeholder="0.00" />
                    </td>
                    <td className="px-1 py-1.5">
                      <input type="number" min="0" step="0.01" className="input py-1.5 text-sm text-right w-full" value={line.credit_amount} onChange={(e) => autoFillDiff(i, "credit_amount", e.target.value)} placeholder="0.00" />
                    </td>
                    <td className="px-1 py-1.5">
                      <input className="input py-1.5 text-sm w-full" value={line.narration} onChange={(e) => setLine(i, "narration", e.target.value)} placeholder="Optional" />
                    </td>
                    {showGst && (
                      <>
                        <td className="px-1 py-1.5">
                          <input type="number" min="0" step="0.5" className="input py-1.5 text-sm text-right w-full" value={line.gst_rate} onChange={(e) => setLine(i, "gst_rate", e.target.value)} placeholder="0" />
                        </td>
                        <td className="px-1 py-1.5">
                          <input type="number" min="0" step="0.01" className="input py-1.5 text-sm text-right w-full" value={line.tds_amount} onChange={(e) => setLine(i, "tds_amount", e.target.value)} placeholder="0.00" />
                        </td>
                      </>
                    )}
                    <td className="px-1 py-1.5 text-center">
                      {lines.length > 2 && (
                        <button onClick={() => removeLine(i)} className="text-red-400 hover:text-red-600 text-xl leading-none">
                          ×
                        </button>
                      )}
                    </td>
              ...
              <tfoot>
                <tr className={`font-semibold text-sm border-t-2 ${balanced ? "bg-green-50 border-green-300" : "bg-red-50 border-red-300"}`}>
                  ...
                  <td className={`px-2 py-2 text-right font-bold ${balanced ? "text-green-700" : "text-red-600"}`}>₹{totalDr.toFixed(2)}</td>
                  <td className={`px-2 py-2 text-right font-bold ${balanced ? "text-green-700" : "text-red-600"}`}>₹{totalCr.toFixed(2)}</td>
                  <td colSpan={showGst ? 4 : 2} className="px-3 py-2">
                    {balanced ? (
                      <span className="text-green-700 font-semibold">✓ Balanced</span>
                    ) : totalDr > 0 || totalCr > 0 ? (
                      <span className="text-red-600 font-semibold">
                        ✗ Difference ₹{diff.toFixed(2)} —{totalDr > totalCr ? ` add ₹${diff.toFixed(2)} Cr` : ` add ₹${diff.toFixed(2)} Dr`}
                      </span>
                    ) : null}
                  </td>
                </tr>
        ...
        {(totalDr > 0 || totalCr > 0) && !balanced && (
          <div className="flex items-start gap-2 px-4 py-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">
            ...
            <div>
              ...
              <span>
                Debit ₹{totalDr.toFixed(2)} vs Credit ₹{totalCr.toFixed(2)}
              </span>
              <p className="text-xs text-red-400 mt-0.5">
                Double-entry rule: every debit must have an equal credit.
                {totalDr > totalCr ? ` Add a credit entry of ₹${diff.toFixed(2)} to balance.` : ` Add a debit entry of ₹${diff.toFixed(2)} to balance.`}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-3 pt-2 border-t border-gray-100">
          <button className="btn-primary px-8 disabled:opacity-40 disabled:cursor-not-allowed" disabled={!balanced || saveMutation.isPending || (isEdit && !!existing?.source_type)} onClick={handleSubmit}>
            {saveMutation.isPending ? "Saving…" : isEdit ? "Update Voucher" : "Save & Post"}
          </button>
          <button className="btn-secondary" onClick={() => navigate("/accounting/vouchers")}>
            Cancel
          </button>
          {!balanced && totalDr > 0 && <span className="text-xs text-gray-400 self-center">Balance the entry above to enable saving</span>}
        </div>
      </div>
