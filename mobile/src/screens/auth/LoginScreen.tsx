...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, KeyboardAvoidingView, Platform, ActivityIndicator, ScrollView, Modal, FlatList, SafeAreaView } from "react-native";
...
export default function LoginScreen() {
  ...
  const handleBiometric = async () => {
    ...
    if (!ok) {
      toast.info("Biometrics not supported on this device");
      return;
    }
  ...
  // ── Country search ──────────────────────────────────────────────────────────
  const filteredCountries = useMemo(() => (search.trim() ? COUNTRIES.filter((c) => c.name.toLowerCase().includes(search.toLowerCase()) || c.dial.includes(search) || c.code.toLowerCase().includes(search.toLowerCase())) : COUNTRIES), [search]);
        ...
        <View style={styles.card}>
          ...
          {tab === "phone" && phoneStep === "phone" && (
            <View>
              ...
              <View style={styles.phoneRow}>
                ...
                <TextInput style={styles.phoneInput} placeholder="Phone number" keyboardType="phone-pad" value={phone} onChangeText={setPhone} autoFocus />
              </View>
          ...
          {tab === "email" && (
            <View>
              ...
              <TextInput style={styles.textInput} placeholder="you@example.com" keyboardType="email-address" autoCapitalize="none" autoCorrect={false} value={email} onChangeText={setEmail} autoFocus />

              <Text style={[styles.label, { marginTop: 14 }]}>Password</Text>
              <View style={styles.passwordRow}>
                <TextInput style={styles.passwordInput} placeholder="Your password" secureTextEntry={!showPassword} value={password} onChangeText={setPassword} autoCapitalize="none" returnKeyType="go" onSubmitEditing={handleEmailLogin} />
          ...
          <View style={styles.socialRow}>
            <TouchableOpacity style={styles.socialBtn} onPress={() => promptGoogle()} disabled={!googleRequest || socialLoading !== null}>
            ...
            <TouchableOpacity style={styles.socialBtn} onPress={() => promptFacebook()} disabled={!fbRequest || socialLoading !== null}>
      ...
      <Modal visible={pickerVisible} animationType="slide" onRequestClose={() => setPickerVisible(false)}>
        <SafeAreaView style={styles.modal}>
          ...
          <View style={styles.searchBox}>
            ...
            <TextInput style={styles.searchInput} placeholder="Search country or dial code…" value={search} onChangeText={setSearch} autoFocus clearButtonMode="while-editing" />
          </View>
          <FlatList
